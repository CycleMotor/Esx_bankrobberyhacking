...
AddEventHandler("bankrobberies:eventHandler", function(event, eventData)
	...
	elseif event == "alarm_police" then
		if ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "police" then
			...
			ESX.ShowNotification("Alguém está a tentar ~r~hackear~s~o banco - ~g~ " .. eventData .. " ~s~localização  marcada.")
		...
		for bank, values in pairs(Config.Bank) do
			...
			if dstCheck <= 5.0 then
				...
				if dstCheck <= 1.0 then
					local usable, displayText = not cachedData["banks"][bank], cachedData["hacking"] and "~r~Hacking..." or cachedData["banks"][bank] and "~r~Assalto~s~ em progresso . . ." or "~INPUT_CONTEXT~ ~r~Invadir~s~ o dispositivo."
