...
AddEventHandler("bankrobberies:receiveCash", function()
	...
	if player then
		...
		TriggerClientEvent("esx:showNotification", source, "Recebeste~r~~n~" .. randomMoney .. "~s~€ ~r~dinheiro sujo")
		else
		player.addMoney(randomMoney)
		TriggerClientEvent("esx:showNotification", source, "Recebes-te~g~ " .. randomMoney .. "~s~€")
		end
